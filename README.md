...
Node v22.21.0、pnpm v10.18.3 を用いて開発されています。
...
```bash
EDITOR_VERSION="0.25.2"
RESOURCE_VERSION="0.25.2"
NEMO_VERSION="0.24.0"
...
フォントは配信容量削減のため、使用する文字だけを含むフォントサブセットを作っています。
生成済みのフォントをキャッシュしておき、元フォントと使用文字が同じ場合は再利用します。

サブセット生成にはWOFF2に対応したfontToolsの`pyftsubset`が必要です。
